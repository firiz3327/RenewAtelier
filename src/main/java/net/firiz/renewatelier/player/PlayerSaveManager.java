...
public enum PlayerSaveManager {
    ...
    private final String stringAccounts = "accounts";
    ...
    public void loadPlayers() {
        Bukkit.getWorlds().forEach(world -> world.getPlayers().forEach(player -> loadStatus(player.getUniqueId())));
    }
    ...
    public void loadStatus(final UUID uuid) {
        ...
        List<List<Object>> select = sql.select(stringAccounts, columns, wheres);
        if (select.isEmpty()) {
            sql.insert(stringAccounts, "uuid", uuid.toString());
            select = sql.select(stringAccounts, columns, wheres);
        }
        //</editor-fold>
        ...
        final PlayerStatus status = new PlayerStatus(
                ...
                alchemy_level,
                alchemy_exp,
                CollectionUtils.castList(objs.get(0)), // recipeStatus
                CollectionUtils.castList(objs.get(1)), // questStatus
                CollectionUtils.castList(objs.get(2)) // discoveredRecipes
        );
        new Thread(() -> {
